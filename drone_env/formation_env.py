...
class FormationAviaryEnv(gym.Env):
    ...
    def __init__(self, config: FormationEnvConfig | None = None):
        ...
        self._goal_center = np.array([0.0, 0.0, self.cfg.desired_height], dtype=np.float32)
        self._formation_offsets = self._build_line_formation_offsets(
            self.cfg.num_drones, self.cfg.formation_spacing
        )
    ...
    def _compute_reward(self, raw_obs: Any, action: np.ndarray) -> float:
        ...
        track_cost = np.mean(np.linalg.norm(pos - desired, axis=1))
        vel_cost = np.mean(np.linalg.norm(vel, axis=1))
        act_cost = np.mean(action ** 2)
        safe_cost = float(np.sum(np.maximum(0.0, self.cfg.safe_distance - pairwise)))

        return -1.2 * track_cost - 0.05 * vel_cost - 0.02 * act_cost - 2.0 * safe_cost
